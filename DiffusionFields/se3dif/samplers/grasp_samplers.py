...
from DiffusionFields.se3dif.utils import SO3_R3
...
class Grasp_AnnealedLD():
    def __init__(self, model, device='cpu', batch=10, dim =3, k_steps=1,
                 T=200, T_fit=5, deterministic=False, model_input=2):
    ...
    def _step(self, H0, t, noise_off=True):
        ...
        if self.model_input==2:
            return H1, t_in
        else:
            return H1

    def sample(self, save_path=False, batch=None,model_input=2):
        #self.model_input=model_input
        ...
        if self.model_input==1:
            ...
            if save_path:
                trj_H = Ht[None,...]
            for t in tqdm(range(self.T)):               ### cgdf ### makes the sampling progress visible using tqdm... can do without it as well.
                Ht = self._step(Ht, t, noise_off=self.deterministic)
                if save_path:
                    trj_H = torch.cat((trj_H, Ht[None,:]), 0)
