...
class ConvGraspDiffusionFields(GraspDiffusionFields):
    ...
    def __init__(self, vision_encoder, vision_decoder, geometry_encoder, points, feature_encoder, decoder):
    ...
    def compute_sdf(self, x):
        #x=x.view(4, 1000, 3)
        latent_vec = self.get_latent(local_context=x.view(4, 1000, 3))
    ...
    def part_forward(self,p,k,batch=1):
        k_ext = k.unsqueeze(1).repeat(1, p.shape[1])
        z_ext = self.get_latent(local_context=p.reshape(batch, -1, p.shape[-1]))
    ...
    def forward(self, H, k, batch=1):
        ## 1. Represent H with points
        p = self.geometry_encoder(H, self.points)
        return self.part_forward(p, k, batch=batch)
