...
class GraspLatentDDM(BaseGraspSampler):
    def __init__(
        ...
        # inititate latent space for simulation
        self.z=None
    ...
    @torch.no_grad()
    def set_latent(self, xyz=None, num_grasps=10, return_intermediate=False, **kwargs):
        ...
        z_pc_cond = self.vae_model.encode_pc(xyz.to("cuda:0"))

        # Repeat interleave latents as per num_grasps
        self.z = z_pc_cond.repeat_interleave(num_grasps, dim=0)

        

    def generate_grasps(self, return_intermediate=False, **kwargs):
