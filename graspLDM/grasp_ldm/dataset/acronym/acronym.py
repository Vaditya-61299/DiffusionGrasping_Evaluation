...
from graspLDM.grasp_ldm.utils.rotations import H_to_tmrp, PoseRepresentation
from graspLDM.grasp_ldm.utils.torch_utils import minmax_normalize
...
class AcronymBaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.acronym_grasps_dir = os.path.join(self.root_dir, "grasps")#,f"{obj_class}")
        self.mesh_dir = os.path.join(self.root_dir, "meshes")

        #new_filter_categories=["Cup", "Mug", "Bowl", "Laptop", "Bag", "Banana", "Camera", "Hammer", "PowerSocket", "PowerStrip", "PS3", "PSP", "Ring", "Shampoo", "SodaCan", "ToiletPaper", "ToyFigure", "Wallet", "Pizza", "RubiksCube", "Tank", "USBStick"]      #works because these splits are avilabe
        new_filter_categories=[]
        new_filter_categories.append(obj_class)
        ...
        if new_filter_categories:
            self._set_filtered_category_list(new_filter_categories)
        else:
...
#        print ("mesh path: ", mesh_path)
    ...
    def _load_all_obj_grasps(self):
        ...
        for i, category in enumerate(tqdm.tqdm(
            ...
            for grasp_filename in category[self.split]:
                ...
                grasp_fp = os.path.join(self.acronym_grasps_dir,self.category_list[i] ,grasp_filename)      #added self.category_list
