...
import numpy as np
from DiffusionFields.se3dif.utils.torch_utils import to_torch
...
class AcronymShapenetPointclouds(AcronymBaseDataset):
    ...
    def __init__(
        ...
        rotation_repr="H",      #originally mrp
        augs_config=None,
        ...
        self._set_normalization_params(use_dataset_statistics_for_norm)            ###CHANGEDD###
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        '''if self.eval==True:
            ...
            self.net_scale=8.0
            self.center_P=True
            #rot = to_torch(R.from_quat(self.q).as_matrix(), device="cpu")
            #pc = torch.einsum('mn,bn->bm', rot, pc)
            self.P = pc.clone()


            pc *=self.net_scale
            if self.center_P:
                self.P_mean = torch.mean(pc, 0)
                pc += -self.P_mean
            if self.center_P:
                pass'''
                #grasps[:, :3] = grasps[:, :3] + self.P_mean
            
        #                print("H_episode after: ", H_episode)
            #grasps[:, :3] = grasps[:, :3]/self.net_scale
        ...
        '''if self.eval==True:
            return pc'''
        metas.update(preproc_metas)        #CHANGED##
