...
from graspLDM.grasp_ldm.dataset.builder import build_dataset_from_cfg
from graspLDM.grasp_ldm.models.builder import build_model_from_cfg
from graspLDM.grasp_ldm.utils.config import Config
from graspLDM.grasp_ldm.utils.rotations import tmrp_to_H
from graspLDM.grasp_ldm.utils.torch_utils import fix_state_dict_prefix
from graspLDM.grasp_ldm.utils.vis import visualize_pc_grasps
...
class Inference:
    ...
    def build_dataset(self, config, split: str = "test", obj_class="Mug"):
...
class InferenceLDM(Inference):
    def __init__(
        ...
        if load_dataset:
            ...
            self.dataset = self.build_dataset(config=self.config, split=data_split,obj_class=obj_class)
    ...
    def generate_grasps(self, pc, metas, num_grasps=10, cls_cond=None, **kwargs):
        ...
        # start = time.time()
        self.z = self.model.set_latent(num_grasps=num_grasps,**in_kwargs)
        ...
        #H=self.grasps_for_simulation(pc,)
        ...
        ### For Simulation ###
        self.batch_pcs=batch_pcs
        self.metas=metas
        self.num_pcs_in=num_pcs_in
        self.final_grasps=final_grasps
        self.tmrp=tmrp
        self.cls_logits=cls_logit
        self.confidence=confidence
        self.qualties=qualities
        self.pc=pc
        self.grasp_unnorm=grasp_unnorm
        self.H_grasps=H_grasps
    ...
    def for_simulation(self,final_grasps,num_grasps):
        final_grasps=final_grasps
        num_pcs_in=self.num_pcs_in
        num_grasps=num_grasps
        batch_pcs=self.batch_pcs
        metas=self.metas
...
class InferenceVAE(Inference):
    ...
    def generate_grasps(self, pc, metas, num_grasps=10, **kwargs):
        ...
        # Infer
        final_grasps = self.model.generate_grasps(batch_pcs, num_grasps)   #TODO from here
